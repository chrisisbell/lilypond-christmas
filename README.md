...
By default, Frescobaldi generates files containing Point and Click information, which is useful whilst writing, but produces a considerably larger output file. For publishing, long click on the LilyPond button and select `Engrave (publish)`. If using LilyPond from the command line, add the option `-dpoint-and-click='#f'`.
